...
const MenuItemDetails = () => {
    ...
    const fetchItemDetails = async () => {
      try {
        ...
        setItem({
          ...
          Description: 'Classic fresh tomato sauce, creamy mozzarella cheese, and aromatic fresh basil leaves baked to perfection in a stone oven.',
          Category: 'Main Course',
          Price: 12.99,
          IsAvailable: true,
          Image: 'https://images.unsplash.com/photo-1604382354936-07c5d9983bd3?w=800',
        });
        setError(err.response?.data?.message || 'Loaded mock item preview (Backend offline)');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!item) {
    return (
      <div className="text-center py-12">
        <h2 className="text-2xl font-bold text-gray-800">Menu item not found</h2>
        <Link to="/" className="text-amber-600 font-medium hover:underline mt-4 inline-block">
          &larr; Back to Menu
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
      <Link to="/" className="text-amber-600 font-medium hover:underline text-sm inline-block mb-6">
      ...
      <div className="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden grid grid-cols-1 md:grid-cols-2">
        ...
        <div className="p-8 flex flex-col justify-between">
          <div>
            ...
            <h1 className="text-3xl font-extrabold text-gray-900 mt-3">{item.Name}</h1>
          ...
          {!isAdmin ? (
            ...
            <div className="mt-8 pt-6 border-t border-gray-100">
              <p className="text-xs text-gray-400 text-center italic">
                Admin view mode: Ordering is disabled for administrator accounts.
              </p>
            </div>
          )}
        </div>
